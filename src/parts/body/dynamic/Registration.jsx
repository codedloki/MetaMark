...
export default function Register() {
  ...
  // --- GAS FEE HELPER ---
  // Fetches dynamic fees and ensures they meet Polygon Amoy's minimum 25 Gwei tip
  ...
  const registerCustomer = async (e) => {
    ...
    try {
      ...
      const tx = await registry.registerCustomer(
        ...
        Number(customdata.accType), 
        customdata.contact,
        { ...fees } // Spread dynamic fees into overrides
      );
      await tx.wait();
      window.location.reload();
    } catch (err) { 
      console.error(err);
      alert(err.reason || "Registration Failed. Check console for details."); 
    } finally { setLoading(false); }
  };
  ...
  // ... (Keep the existing UI/JSX code below as it was)
  if (!isConnected) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center p-6 text-center space-y-6 bg-[#020617]">
        <div className="bg-blue-500/10 p-6 rounded-full animate-bounce border border-blue-500/20">
        ...
        <CardContent className="px-6 pb-10">
          <Tabs defaultValue="manufacturer" className="w-full">
            ...
            <TabsContent value="consumer" className="space-y-5">
              ...
              <Button 
                className="w-full h-14 text-xs font-black uppercase tracking-widest bg-blue-600 text-white hover:bg-blue-500 rounded-2xl shadow-xl transition-all active:scale-95 border-none mt-2" 
                disabled={!custermstate || loading} 
                onClick={registerCustomer}
              >
                 {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Complete Consumer Setup"}
